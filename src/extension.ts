...
function inspectProjectRoot(): ProjectRootInfo {
  ...
  if (resolved.error === 'no-workspace' || !resolved.rootPath) {
    return {
      ...
      message: 'Open a folder that contains your Android project, or choose the project root with the Project gear.',
    };
  }
  if (!fs.existsSync(resolved.rootPath) || !fs.statSync(resolved.rootPath).isDirectory()) {
    return {
      ...
      message: `Project root not found: ${resolved.displayPath}. Choose a different folder with the Project gear.`,
    };
  }
  ...
  if (!fs.existsSync(gradleWrapperPath(uri))) {
    return {
      ...
      message: `No Gradle wrapper in ${displayPath}. Choose the Android project directory that contains gradlew.`,
    };
  }
...
function gradleWrapper(root: vscode.Uri): string {
  ...
  if (!fs.existsSync(wrapper)) {
    throw new Error(`Gradle wrapper not found: ${wrapper}. Choose the Android project directory that contains gradlew.`);
  }
  return wrapper;
}
